#!/usr/bin/env node
// Claude Code Stop hook for one bb finish pipeline. Zero dependencies. Node 16+.

import { readFileSync, unlinkSync } from "node:fs";
import { resolve } from "node:path";
import {
  FINISH_DIR, gateState, hookStatePath, parseGates, qualify, readStableRegularFile,
...
if (scopeIndex !== -1 && (!scopeArg || validateScopeId(scopeArg))) {
  allow("bb finish: installed hook has an invalid --scope value; not blocking.");
}
...
if (target.ambiguous) {
  allow("bb finish: " + target.ambiguous.length + " pipelines under " + FINISH_DIR +
    "/ (" + target.ambiguous.join(", ") + ") and none bound to this session; not blocking.");
}
if (target.error && !target.ambiguous) allow("bb finish: " + safeHostText(target.error) + "; not blocking.");
...
if (!unmet.length && !invalid.length) {
  ...
  allow("bb finish" + where + ":" + handoffMessage());
}
...
try {
  ...
  if (discoveryErrors.length) {
    console.log(JSON.stringify({
      decision: "block",
      reason: "bb finish: invalid named scope input must be repaired before Stop: " +
        safeHostText(discoveryErrors[0]),
    }));
    process.exit(0);
  }
  allow("bb finish: could not update the serialized hook state (" + safeHostText(error.message) + "); not blocking to avoid a trap.");
}

const where = target.scope ? " [scope " + target.scope + "]" : "";
const outstanding = [...invalid, ...unmet].map((item) => safeHostText(item));
if (sessionState.blocks > MAX_BLOCKS) {
  allow("bb finish: releasing after " + MAX_BLOCKS + " blocks without gate progress" + where +
...
console.log(JSON.stringify({
  decision: "block",
  reason: "bb finish" + where + ": " + outstanding.length + " gate/ledger/dispatch item(s) need work: " + list +
