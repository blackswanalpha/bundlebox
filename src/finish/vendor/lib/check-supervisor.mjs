...
if (!shell || command === undefined) {
  console.error("bb finish-check-supervisor: expected resolved shell and CHECK command");
  process.exit(2);
}
...
try {
  ...
  console.error("bb finish-check-supervisor: could not start CHECK: " + error.message);
  process.exit(127);
}
...
child.once("close", (code, signal) => {
  if (spawnError) {
    console.error("bb finish-check-supervisor: CHECK spawn failed: " + spawnError.message);
  ...
  console.error("bb finish-check-supervisor: CHECK ended by " + (signal || "unknown signal"));
  process.exitCode = 1;
});
