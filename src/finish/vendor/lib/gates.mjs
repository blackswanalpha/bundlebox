...
export const FINISH_DIR = ".bundlebox/finish";
export const LOCK_DIR = join(FINISH_DIR, "locks");
...
export function scopeRoot(root, scope) {
  return join(root, FINISH_DIR, scope);
}

export function listScopes(root) {
  const directory = join(root, FINISH_DIR);
...
export function resolveTarget(options = {}) {
  ...
  const wanted = options.scope || process.env.BB_FINISH_SCOPE || null;
  if (wanted) {
    ...
    if (!scopes.includes(wanted)) {
      ...
      const statePath = join(root, FINISH_DIR);
      ...
      return {
        ...
        error: "no such scope \"" + wanted + "\" under " + FINISH_DIR + "/ (have: " +
          (scopes.join(", ") || "none") + ")",
      };
    }
    return targetFromDiscovery("scope", wanted, scopeDiscovery(root, wanted));
  ...
  if (scopes.length > 1) {
    ...
    return {
      ...
      error: scopes.length + " pipelines present (" + scopes.join(", ") +
        "); pass --scope <id> or set BB_FINISH_SCOPE. Refusing to guess.",
    };
  }
...
export function statusLogPath(root, scope) {
  return scope ? join(scopeRoot(root, scope), "status.log") : join(root, "bb finish-status.log");
}

export function hookStatePath(root, scope) {
  return scope ? join(scopeRoot(root, scope), "hook-state.json") : join(root, ".bundlebox/finish-hook-state.json");
}

function assertSafeStatePath(root, target) {
  const stateRoot = join(resolve(root), FINISH_DIR);
...
function lockDirectory(root) {
  ...
  // namespace. The root itself must exist for any bb finish operation.
