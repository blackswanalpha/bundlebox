...
import {
  MAX_AUTOMATIC_EVIDENCE_CHARS, MAX_CHECK_OUTPUT_BYTES, FINISH_DIR, appendStatus,
...
run modes:
  ...
  --shell PATH          command shell (BB_FINISH_SHELL, then platform default)
...
pipeline actions:
  ...
  --list-scopes                      list .bb finish pipelines

targeting:
  --scope ID             use .bundlebox/finish/ID (or BB_FINISH_SCOPE)
...
Approvals live outside the repository under ~/.bundlebox/finish/approved by default.
...
if (action === "--list-scopes") {
  ...
  } else console.log("(no pipelines under " + FINISH_DIR + "/)");
  process.exit(0);
}
...
if (!target.files.length && action !== "--release") {
  failUsage("no gate files found (looked for " + FINISH_DIR + "/<scope>/, then GATES.md and gates/*.md under " + root + ")");
}
...
function resolveShell(raw) {
  const requested = raw || process.env.BB_FINISH_SHELL || (process.platform === "win32" ? (process.env.ComSpec || "cmd.exe") : "/bin/sh");
...
const approvalDir = opt.status ? null
  : resolve(process.env.BB_FINISH_APPROVAL_DIR || join(homedir(), ".bundlebox/finish", "approved"));
const canonicalRoot = opt.status ? null : realpathSync(root);
if (!opt.status && pathIsInside(root, approvalDir)) failUsage("BB_FINISH_APPROVAL_DIR must be outside the repository root");
