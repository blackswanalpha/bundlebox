...
node scripts/gate-lint.mjs --strict --json .bundlebox/finish/<scope>/gates/leaf-1.1.1.md
```
...
`bb finish reverify` re-run the entire tree instead of the one leaf that returned.
...
regression across the joined work. ``bb finish template node`` reserves `N1`
