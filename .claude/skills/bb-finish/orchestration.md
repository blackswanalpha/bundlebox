...
5. **Verify each return independently.** Record the native return in its wave, then re-run the returned leaf's runnable gates, including already checked gates:
   ...
   ```text
   bb finish check --root . --cwd . --reverify .bundlebox/finish/<scope>/gates/leaf-1.2.1.md
   ```

   `bb finish status` alone is not re-verification. If an approved oracle changed, inspect it and approve the new oracle before continuing. Review manual gates directly and try to refute at least one passed gate.
6. **Append status and roll forward.** Record the result without rewriting history:

   ```text
   bb finish check --scope <scope> --log "leaf-1.2.1 verified"
   ```

   Mark the leaf `VERIFIED`, release that exact leaf lease, and record the release before promotion:

   ```text
   bb finish check --scope <scope> --leaf leaf-1.2.1 --release
   bb finish check --scope <scope> --log "leaf-1.2.1 lease released"
   ```
...
```text
bb finish check --root . --cwd . --reverify --jobs 4 .bundlebox/finish/<scope>/gates/leaf-1.1.1.md .bundlebox/finish/<scope>/gates/leaf-1.1.2.md
```
...
2. **Parent `bb finish reverify`:** executes each runnable oracle again instead of trusting old or manually written evidence.
