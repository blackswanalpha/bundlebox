#!/usr/bin/env bash
# @lathe applied
# @lathe-id 644e9037c904
# @lathe-applied 2026-09-20T11:01:24.156Z
# @tag lathe
# @title scan → oversight scan → compile → route → snapgen build → arc build → oversight guidelines → pinpoint gaps → buckmaster recommend → tokens ledger → session list → buckmaster episodes → bench init → bench run → tokens calibrate → echos → buckmaster signals → buckmaster rules → buckmaster memory → lathe learn → lathe build → tokens ledger → monitor status → commandcenter build
# @needs bb
# @turns 24
# @cost 0
# @safe false
#
# PROPOSED by LATHE-1 from 87 occurrence(s) across 87 session(s)
# lift 21.2 over the base rate of `commandcenter build`; confidence 0.5404 that the run continues this way once it has started.
#
