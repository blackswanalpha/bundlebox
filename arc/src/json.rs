//! Hand-written JSON, copied from the kernel crate (kernel/src/json.rs).
//!
//! Copied and not shared: arc is a separate crate so it can be built, shipped
//! and versioned on its own, and a two-crate workspace to share 107 lines of
//! parser would make the offline build story worse than the duplication does.
...
impl Json {
    ...
    #[allow(dead_code)]   // kept so this file stays diffable against kernel/src/json.rs
